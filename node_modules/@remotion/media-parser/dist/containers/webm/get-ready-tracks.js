...
import { getTrack, NO_CODEC_PRIVATE_SHOULD_BE_DERIVED_FROM_SPS, } from './make-track';
import { getMainSegment, getTracksSegment } from './traversal';
export const getTracksFromMatroska = ({ state, }) => {
...
export const matroskaHasTracks = (state) => {
    const structure = state.getMatroskaStructure();
    const mainSegment = getMainSegment(structure.boxes);
    if (!mainSegment) {
        return false;
