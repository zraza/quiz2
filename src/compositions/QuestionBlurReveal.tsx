...
/**
 * QuestionBlurReveal — BONUS variant (cinematic dark theme).
 *
 * Dark navy/black gradient background with subtle star shimmer.
 * Image starts at blur(25px) → blur(0px), scale(1.08) → scale(1.0).
 * Glass-frost overlay fades as blur reduces.
...
export const QuestionBlurReveal: React.FC<{ question: BlurRevealQ }> = ({ question }) => {
  ...
  const blurAmount = interpolate(blurProgress, [0, 1], [25, 0]);
  // Scale to hide blur edges (1.08 → 1.0)
  const imgScale = interpolate(blurProgress, [0, 1], [1.08, 1.0]);

  // Glass-frost overlay opacity (fades as blur reduces)
  const frostOpacity = interpolate(blurProgress, [0, 0.7, 1], [0.35, 0.1, 0], CLAMP);
  ...
  return (
    <AbsoluteFill>
      ...
      {/* BLURRED IMAGE — 70% screen height, centered */}
      <div style={{
        position: 'absolute',
        top: 190,
        left: 80,
        ...
        <div style={{
          ...
          {/* Glass-frost overlay that fades as blur reduces */}
          {frostOpacity > 0 && (
            <div style={{
              position: 'absolute',
              inset: 0,
              background: `linear-gradient(135deg, rgba(200, 220, 255, ${frostOpacity * 0.4}), rgba(150, 180, 220, ${frostOpacity * 0.2}))`,
